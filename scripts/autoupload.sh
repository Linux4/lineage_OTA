...
hub -C $REPODIR release create -a $BUILDDIR/$RECOVERY_NAME -a $REPODIR/$RECOVERY_NAME.sha256 -a $ZIP -a $REPODIR/$FILENAME.sha256 -m "$DEVICE: $DATE" -t $(git -C $REPODIR rev-parse HEAD) $RELEASENAME
